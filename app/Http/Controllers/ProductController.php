...
public function rawmterialstore(Request $request)
    {
        ...
        // optional: redirect back with a flash and maybe anchor to modal
        return back()->with('success', 'Raw material created: '.$rm->name);
    }

public function productmaterialshow(Product $product)
    {
        $product->load(['rawMaterials' => function ($q) { $q->orderBy('name'); }]);

        // cost summary
        $materials = $product->rawMaterials->map(function($m){
            $qty = (float)$m->pivot->quantity;
            $price = (float)$m->pivot->unit_price;
            $wastage = (float)$m->pivot->wastage_pct;
            $effectiveQty = $qty * (1 + $wastage/100);
            $lineTotal = $effectiveQty * $price;
            return [
                'model' => $m,
                'qty' => $qty,
                'unit' => $m->pivot->unit,
        ...
        $totals = [
            'materials_cost' => $materials->sum('line_total'),
        ];
            ...
            foreach ($products as $key=>$product)
            {
                ...
                if(in_array("products-edit", $request['all_permission']))
                    $nestedData['options'] .= '<li>
                        ...
                        $nestedData['options'] .= '<li>
                            <a href="'.route('catalog.show',$product->id).'" class="btn btn-link"><i class="fa fa-book"></i> '.__('Batching Sheet').'</a>
                        </li>';
        ...
        try {
            while ($columns = fgetcsv($file)) {
                if (count($escapedHeader) !== count($columns)) {
                    fclose($file);
                    return redirect()->back()->with('message', __('db.CSV file format is incorrect'));
                }
                ...
                if (!$lims_unit_data) {
                    fclose($file);
                    return redirect()->back()->with('not_permitted', __('db.Unit code does not exist in the database'));
                }
                ...
                if (!empty($data['variantvalue']) && !empty($data['variantname'])) {
                    ...
                    foreach ($variantInfo as $key => $info) {
                        if (!strpos($info, "[")) {
                            fclose($file);
                            return redirect()->back()->with('message', __('db.Invalid variant value format'));
                        }
