    ...
    public function store(StorePurchaseRequest $request)
{
    ...
    try {
        ...
        if (!isset($data['reference_no'])) {
            $data['reference_no'] = 'EZ-REF-' . date("Ymd") . '-' . date("his");
        }
        ...
        foreach ($product_id as $i => $id) {
            ...
            if ($lims_product_data->is_variant) {
                ...
                if ($product_purchase['product_batch_id']) {
                    // price probe
                    $lims_product_warehouse_priced = Product_Warehouse::where([
                        ['product_id', $id],
                        ['warehouse_id', $data['warehouse_id']],
                    ])->whereNotNull('price')->select('price')->first();
                    if ($lims_product_warehouse_priced) $price = $lims_product_warehouse_priced->price;
                    else $price = null;
            ...
            if ($lims_product_warehouse_data) {
                ...
                if ($price) $lims_product_warehouse_data->price = $price;
                if ($lims_product_data->is_variant)
                    $lims_product_warehouse_data->variant_id = $lims_product_variant_data->variant_id ?? null;
            }

            // IMEI duplicate checks
            if ($imei_numbers[$i]) {
            ...
            $product_purchase['etd_date']         = $etd_date[$i];
            $product_purchase['moq']              = $moq[$i];
            $product_purchase['ship_term']        = $ship_term[$i];
    ...
    {  
        ...
        if($role->hasPermissionTo('purchases-add')){
            ...
            return view('backend.purchase.import', compact('lims_customer_list', 'lims_warehouse_list', 'lims_tax_list'));
        }
        else
            return redirect()->back()->with('not_permitted', __('db.Sorry! You are not allowed to access this module'));
    }
    ...
    //     }
    // }
protected function normalizeDateYmd(?string $s): ?string
{
    $s = trim((string)$s);
    if ($s === '') return null;

    // Common separators ko standardize
    $s = str_replace(['.', ' '], ['/', ''], $s);
...
public function importPurchase(Request $request)
{
    ...
    if (!$handle) return back()->with('not_permitted', 'Unable to read the uploaded file.')->withInput();

    $header = fgetcsv($handle);
    if (!$header) return back()->with('not_permitted', 'CSV seems empty or invalid.')->withInput();

    $map = [];
    foreach ($header as $idx => $name) $map[strtolower(trim($name))] = $idx;

    $required = ['product_code','quantity'];
    $lineErrors = [];
    foreach ($required as $rh) if (!array_key_exists($rh, $map)) $lineErrors[] = "Header '{$rh}' is required.";
    if ($lineErrors) {
        return back()->with('not_permitted', "Missing required CSV header(s).")
                     ->with('line_errors', $lineErrors)->withInput();
    }
    ...
    while (($cols = fgetcsv($handle)) !== false) {
        ...
        $productCode = trim($cols[$map['product_code']] ?? '');
        $qtyStr      = trim($cols[$map['quantity']] ?? '');
        $discStr     = array_key_exists('discount',$map) ? trim($cols[$map['discount']] ?? '') : '0';
        $batchNo     = array_key_exists('batch_no',$map) ? trim($cols[$map['batch_no']] ?? '') : '';
        $lotNo       = array_key_exists('lot_no',$map) ? trim($cols[$map['lot_no']] ?? '') : '';
        $expiredDateRaw = array_key_exists('expired_date',$map) ? trim($cols[$map['expired_date']] ?? '') : '';
        $moqStr      = array_key_exists('moq',$map) ? trim($cols[$map['moq']] ?? '') : '';
            // Normalize to YYYY-MM-DD (or null)
        $expiredDate = $this->normalizeDateYmd($expiredDateRaw);
        // Agar user ne koi value di hai lekin parse nahi hui
        if ($expiredDateRaw !== '' && $expiredDate === null) {
         $rowErr[] = "Invalid expired_date format '{$expiredDateRaw}'. Use YYYY-MM-DD or DD/MM/YYYY or MM/DD/YYYY.";
        }

        $rowErr = [];
        if ($productCode === '' || $qtyStr === '') $rowErr[] = "product_code and quantity are required.";

        $qty  = (int) str_replace([',',' '], '', $qtyStr ?: '0');
        $disc = ($discStr === '') ? 0.0 : (float) str_replace([',',' '], '', $discStr);
        $moq  = ($moqStr === '') ? null : (float) str_replace([',',' '], '', $moqStr);

        if ($qty <= 0)  $rowErr[] = "quantity must be > 0.";
        if ($disc < 0)  $rowErr[] = "discount cannot be negative.";

        $product = Product::where([['code',$productCode], ['is_active', true]])->first();
        if (!$product) {
            $rowErr[] = "Product code '{$productCode}' not found or inactive.";
        }

        // Resolve purchase unit from product
        $purchaseUnit = null;
        if ($product) {
            if (!empty($product->purchase_unit_id)) {
                $purchaseUnit = Unit::find($product->purchase_unit_id);
            }
            if (!$purchaseUnit && !empty($product->unit_id)) {
                $purchaseUnit = Unit::find($product->unit_id);
            }
            if (!$purchaseUnit) $rowErr[] = "No purchase unit linked to product '{$productCode}'.";
        }

        // COST from product
        $cost = null;
        if ($product) {
            // prefer $product->cost (standard purchase cost). Fallback to 0 if missing.
            $cost = $product->cost ?? null;
            if ($cost === null) {
                $rowErr[] = "Product '{$productCode}' has no cost set (product.cost is null).";
            } elseif ($cost < 0) {
                $rowErr[] = "Product '{$productCode}' cost cannot be negative.";
            }
        }

        if ($rowErr) {
            $lineErrors[] = "Line {$lineNo}: " . implode(' ', $rowErr);
            continue;
        }
        ...
        $rows[] = [
            ...
            'unit'         => $purchaseUnit,
            'qty'          => $qty,
            'cost'         => (float)$cost,   // from product
            'discount'     => $disc,          // per-unit discount
            'batch_no'     => $batchNo,
            'lot_no'       => $lotNo,
    ...
    if (!empty($lineErrors)) {
        return back()->with('not_permitted', "There were errors in your CSV.")
                     ->with('line_errors', $lineErrors)->withInput();
    }
    ...
    try {
        $data = [
            ...
            'ship_instruction' => $request->input('ship_instruction'),
        ];

        if ($request->filled('customer_id')) {
            $data['supplier_id'] = (int)$request->input('customer_id');
        }

        if ($request->filled('created_at')) {
            $ts = date("Y-m-d H:i:s", strtotime(str_replace("/", "-", $request->input('created_at'))));
            $data['created_at'] = $ts;
            $data['updated_at'] = $ts;
        } else {
            $data['created_at'] = now();
        ...
        if ($reference === '') $reference = 'pr-' . date("Ymd") . '-' . date("His");
        $data['reference_no'] = $reference;

        $poNo = trim($request->input('po_no',''));
        if ($poNo === '') {
            $poNo = method_exists($this, 'generatePoNumber') ? $this->generatePoNumber() : ('PO-' . date("Ymd") . '-' . date("His"));
        }
        $data['po_no'] = $poNo;
        $data['system_po_no'] = method_exists($this, 'generatePoNumber') ? $this->generatePoNumber() : ('SPO-' . date("Ymd") . '-' . date("His"));

        if ($request->hasFile('document')) {
            $document = $request->file('document');
            $ext = $document->getClientOriginalExtension();
            $filename = (config('database.connections.saleprosaas_landlord') ? ($this->getTenantId() . '_') : '') . date("YmdHis") . '.' . $ext;
            $document->move(public_path('documents/purchase'), $filename);
            $data['document'] = $filename;
        }

        $purchase = Purchase::where('reference_no', $data['reference_no'])
            ->where('warehouse_id', $data['warehouse_id'])->first();

        $isAppend = (bool) $purchase;

        if (!$purchase) {
            $purchase = Purchase::create($data);
        } else {
            $purchase->status           = $data['status'];
            $purchase->order_discount   = $data['order_discount'];
            if (!empty($data['note']))             $purchase->note = $data['note'];
            if (!empty($data['comments']))         $purchase->comments = $data['comments'];
            if (!empty($data['ship_instruction'])) $purchase->ship_instruction = $data['ship_instruction'];
            if (!empty($data['supplier_id']))      $purchase->supplier_id = $data['supplier_id'];
            if (!empty($data['document']))         $purchase->document    = $data['document'];
            $purchase->save();
        }
        ...
        foreach ($rows as $r) {
            ...
            $unit    = $r['unit'];
            $qty     = $r['qty'];
            $cost    = $r['cost'];     // from product
            $disc    = $r['discount']; // per-unit
            // variant?
            $variantId = null;
            if ($product->is_variant) {
                $pv = ProductVariant::select('id','variant_id','qty')
                        ->FindExactProductWithCode($product->id, $product->code)
                        ->first();
                if ($pv) $variantId = $pv->variant_id;
            }

            // batch
            $product_batch_id = null;
            if (!empty($r['batch_no'])) {
                $pb = ProductBatch::where([
                        ['product_id', $product->id],
                        ['batch_no', $r['batch_no']],
                        ['lot_no', $r['lot_no']]
                    ])->first();
                if ($pb) {
                    if (!empty($r['expired_date'])) $pb->expired_date = $r['expired_date'];
                    $pb->save();
                } else {
                    $pb = ProductBatch::create([
                        'product_id'   => $product->id,
                        'batch_no'     => $r['batch_no'],
            ...
            if ((int)$purchase->status === 1) {
                $quantityForStock = $qty;
                if ($unit && $unit->operator === '*')       $quantityForStock = $qty * $unit->operation_value;
                elseif ($unit && $unit->operator === '/')   $quantityForStock = $qty / max(1, $unit->operation_value);

                $product->qty = ($product->qty ?? 0) + $quantityForStock;
                $product->save();

                if ($product->is_variant && isset($pv)) {
                    $pv->qty += $quantityForStock;
                    $pv->save();
                }

                $pwQuery = Product_Warehouse::where([
                    ['product_id', $product->id],
                    ['warehouse_id', $purchase->warehouse_id],
                ]);
                if ($product_batch_id) $pwQuery->where('product_batch_id', $product_batch_id);
                if ($variantId)        $pwQuery->where('variant_id', $variantId);

                $pw = $pwQuery->first();
                if ($pw) {
                    $pw->qty = ($pw->qty ?? 0) + $quantityForStock;
                    if ($product_batch_id) $pw->product_batch_id = $product_batch_id;
                    if ($variantId)        $pw->variant_id = $variantId;
                } else {
                    $pw = new Product_Warehouse();
                    $pw->product_id       = $product->id;
                    $pw->warehouse_id     = $purchase->warehouse_id;
                    $pw->qty              = $quantityForStock;
                    $pw->product_batch_id = $product_batch_id;
                    $pw->variant_id       = $variantId;
                }
                $pw->save();

                if ($product_batch_id) {
                    $pb = ProductBatch::find($product_batch_id);
                    if ($pb) { $pb->qty = ($pb->qty ?? 0) + $quantityForStock; $pb->save(); }
                }
            }

            // ProductPurchase line (ETS/ETA/ETD removed → default to now())
            $pp = new ProductPurchase();
            $pp->purchase_id       = $purchase->id;
            $pp->product_id        = $product->id;
            $pp->variant_id        = $variantId;
            $pp->qty               = $qty;
            $pp->recieved          = ((int)$purchase->status === 1) ? $qty : 0;
            $pp->purchase_unit_id  = $unit?->id;
            $pp->net_unit_cost     = number_format((float)$netUnit, $decimal, '.', '');
            $pp->discount          = number_format((float)($disc * $qty), $decimal, '.', '');
            $pp->tax_rate          = $taxRate;
            $pp->tax               = number_format($lineTax, $decimal, '.', '');
            $pp->total             = number_format((float)$lineTotal, $decimal, '.', '');
            $pp->product_batch_id  = $product_batch_id;
            $pp->moq               = $r['moq'];
            // Required datetime columns now auto-set to now()
            $now = now();
            $pp->ets_date          = $now;
            $pp->eta_date          = $now;
            $pp->etd_date          = "";
            ...
            $purchase->total_discount += ($disc * $qty);
            $purchase->total_tax      += 0;
            $purchase->total_cost     += (float) number_format((float)$lineTotal, $decimal, '.', '');
            $purchase->item++;
            $added++;
        }

        $purchase->order_tax   = 0;
        $purchase->grand_total = max(0, ($purchase->total_cost - $purchase->order_discount));
        $purchase->save();

        // simple log
        $logFields = $request->except(['_token','document']);
        $convertedLog = [];
        ...
        foreach ($custom_fields as $cf) {
            ...
            if ($request->has($field_name)) {
                ...
                $custom_field_data[$field_name] = in_array($cf->type, ['checkbox','multi_select'])
                    ? (is_array($val) ? implode(",", $val) : $val)
                    : $val;
            }
        }
        if (!empty($custom_field_data)) {
            DB::table('purchases')->where('id', $purchase->id)->update($custom_field_data);
        }

        DB::commit();

        $msg = $isAppend
            ? "Rows appended to existing purchase ({$added} added)."
            : "Purchases imported successfully ({$added} rows}).";
        return back()->with('message', $msg);

    } catch (\Throwable $e) {
        DB::rollBack();
        return back()->with('not_permitted', 'Transaction failed: ' . $e->getMessage())->withInput();
    }
}


// IMEI helper stays for future use
private function imeiExists(string $imei, int $productId): bool
{
    $imei = trim($imei);
    if ($imei === '') return false;

    $existsInLines = ProductPurchase::where('product_id', $productId)
        ->whereNotNull('imei_number')
        ->whereRaw('FIND_IN_SET(?, REPLACE(imei_number, " ", "")) > 0', [$imei])
        ->exists();

    if ($existsInLines) return true;

    $existsInWarehouse = Product_Warehouse::where('product_id', $productId)
        ->whereNotNull('imei_number')
        ->whereRaw('FIND_IN_SET(?, REPLACE(imei_number, " ", "")) > 0', [$imei])
        ->exists();

    return $existsInWarehouse;
}
    ...
    public function purchaseData(Request $request)
    {
        ...
        if(!empty($purchases))
        {
            foreach ($purchases as $key=>$purchase)
            {
                ...
                $nestedData['key'] = $key;
                $nestedData['date'] = date("m-d-Y", strtotime($purchase->created_at->toDateString()));
                ...
                $nestedData['purchase_status'] = '<div class="badge badge-success">'.__('db.Received').'</div>';
                $purchase_status = __('db.Received');
                }
                elseif ($purchase->status == 2) {
                $nestedData['purchase_status'] = '<div class="badge badge-warning">'.__('db.Partial').'</div>';
                $purchase_status = __('db.Partial');
                }
                elseif ($purchase->status == 3) {
                $nestedData['purchase_status'] = '<div class="badge badge-secondary">'.__('db.Pending').'</div>';
                $purchase_status = __('db.Pending');
                }
                elseif ($purchase->status == 4) {
                $nestedData['purchase_status'] = '<div class="badge badge-info">'.__('db.Ordered').'</div>';
                ...
                $nestedData['purchase_status'] = '<div class="badge badge-danger">'.__('db.Cancel').'</div>';
              ...
              $nestedData['purchase'] = [
    '[ "' . date("d-m-Y", strtotime($purchase->created_at->toDateString())) . '"',
    ...
    public function productPurchaseData($id)
    {
        ...
        $supplier = Supplier::find($product_purchase_data->supplier_id);
                ...
                if($returned_imei_number_data) {
                    ...
                    $product_purchase[12][$key] = $product_purchase_data->ship_term;
...
//                 $product_purchase['ship_term'] = $ship_term[$key] ?? null;
...
//             $product_purchase['etd_date'] = $etd_date[$key];
   ...
   public function update(UpdatePurchaseRequest $request, $id)
{
    ...
    if ($request->hasFile('document')) {
        ...
        $this->fileDelete(public_path('documents/purchase/'), $purchase->document);

        $ext = pathinfo($request->file('document')->getClientOriginalName(), PATHINFO_EXTENSION);
        $documentName = date("Ymdhis");
        if (!config('database.connections.saleprosaas_landlord')) {
            $documentName = $documentName . '.' . $ext;
            $request->file('document')->move(public_path('documents/purchase'), $documentName);
        } else {
            $documentName = $this->getTenantId() . '_' . $documentName . '.' . $ext;
            $request->file('document')->move(public_path('documents/purchase'), $documentName);
        }
    ...
    $data['payment_status'] = ($balance == 0.0) ? 2 : 1;

    // ---------------- Request arrays ----------------
    $product_id     = $request->input('product_id', []);
    $product_code   = $request->input('product_code', []);
    $qty            = $request->input('qty', []);
    $recieved       = $request->input('recieved', $qty);
    ...
    $supplier_ids   = $request->input('supplier_name', []);
    $ets_date       = $request->input('ets_date', []);
    $eta_date       = $request->input('eta_date', []);
    $etd_date       = $request->input('etd_date', []);
    $moq            = $request->input('moq', []);
    $ship_term      = $request->input('ship_term', []);
    ...
    $meaningful = function ($v) {
        if ($v === null || $v === '') return false;
        if ($v === '1970-01-01' || $v === '1970-01-01 00:00:00') return false;
        return true;
    };
    $idToName = function ($field, $value) {
        if (!$value) return null;
        switch ($field) {
            case 'warehouse_id': return optional(Warehouse::find($value))->name;
            case 'customer_id' : return optional(Customer::find($value))->name;
            case 'biller_id'   : return optional(Biller::find($value))->company_name;
            case 'supplier_id' : return optional(Supplier::find($value))->name;
        }
        return null;
    };

    // ---------------- Take ORIGINAL snapshots (for diffs) ----------------
    $origHeader = $purchase->replicate()->getAttributes();

    // line snapshot helper (adds friendly fields: batch_no, lot_no, supplier_name, unit_name)
    $snapLine = function (ProductPurchase $pp) {
        $batch = $pp->product_batch_id ? ProductBatch::find($pp->product_batch_id) : null;
        $unit  = Unit::find($pp->purchase_unit_id);
        return [
            'product_id'       => $pp->product_id,
            'variant_id'       => $pp->variant_id,
            'qty'              => $pp->qty,
            'recieved'         => $pp->recieved,
            'purchase_unit_id' => $pp->purchase_unit_id,
            'purchase_unit'    => $unit ? $unit->unit_name : null,
            'net_unit_cost'    => $pp->net_unit_cost,
            'discount'         => $pp->discount,
            'tax_rate'         => $pp->tax_rate,
            'tax'              => $pp->tax,
            'total'            => $pp->total,
            'supplier_id'      => $pp->supplier_id,
            'supplier_name'    => $pp->supplier_id ? optional(Supplier::find($pp->supplier_id))->name : null,
            'imei_number'      => $pp->imei_number,
            'ets_date'         => $pp->ets_date,
            'eta_date'         => $pp->eta_date,
            'etd_date'         => $pp->etd_date,
            'moq'              => $pp->moq,
            'ship_term'        => $pp->ship_term,
            'ship_cost'        => $pp->ship_cost,
            'product_batch_id' => $pp->product_batch_id,
            'batch_no'         => $batch ? $batch->batch_no : null,
            'lot_no'           => $batch ? ($batch->lot_no ?? null) : null,
            'expired_date'     => $batch ? ($batch->expired_date ?? null) : null,
        ];
    };

    $origLines = ProductPurchase::where('purchase_id', $id)->get();
    $origMap = [];
    foreach ($origLines as $pp) {
        $key = $pp->product_id . ':' . ((int)$pp->variant_id);
        $origMap[$key] = $snapLine($pp);
    }

    // ================= TRANSACTION =================
    DB::beginTransaction();
    try {
        // -------- revert stock & delete old lines --------
        foreach ($origLines as $pp) {
            $oldRecv = $pp->recieved;
            $unit = Unit::find($pp->purchase_unit_id);
            if ($unit) {
                $oldRecv = ($unit->operator == '*')
                    ? $oldRecv * $unit->operation_value
                    : $oldRecv / max(1, $unit->operation_value);
            }

            $product = Product::find($pp->product_id);
            if ($product && $product->is_variant) {
                $pv = ProductVariant::select('id', 'variant_id', 'qty')
                    ->FindExactProduct($product->id, $pp->variant_id)->first();
                $pw = Product_Warehouse::where([
                    ['product_id', $product->id],
                    ['variant_id', $pp->variant_id],
                    ['warehouse_id', $purchase->warehouse_id]
                ])->first();
                if ($pv) { $pv->qty -= $oldRecv; $pv->save(); }
            } elseif ($pp->product_batch_id) {
                $batch = ProductBatch::find($pp->product_batch_id);
                if ($batch) { $batch->qty -= $oldRecv; $batch->save(); }
                $pw = Product_Warehouse::where([
                    ['product_id', $pp->product_id],
                    ['product_batch_id', $pp->product_batch_id],
                    ['warehouse_id', $purchase->warehouse_id],
            ...
            if ($pp->imei_number && isset($pw->imei_number)) {
                $prev = array_filter(array_map('trim', explode(',', $pp->imei_number)));
                $cur  = array_filter(array_map('trim', explode(',', $pw->imei_number)));
                $cur  = array_values(array_diff($cur, $prev));
                $pw->imei_number = count($cur) ? implode(',', $cur) : null;
            }

            if ($product) { $product->qty -= $oldRecv; $product->save(); }
            if ($pw)      { $pw->qty -= $oldRecv; $pw->save(); }
            $pp->delete();
        }

        // -------- add new lines & update stock --------
        for ($i = 0; $i < count($product_id); $i++) {
            $pid = (int)($product_id[$i] ?? 0);
            if (!$pid) continue;

            $product = Product::findOrFail($pid);

            // unit resolve (purchase_unit[] string list => first)
            $unitStr   = $purchase_unit[$i] ?? '';
            $unitName  = trim(explode(',', $unitStr)[0] ?? '');
            $unitModel = Unit::where('unit_name', $unitName)->first();
            if (!$unitModel) {
                DB::rollBack();
                return redirect()->route('purchases.edit', $id)->with('not_permitted', __('db.Invalid unit for product.'));
            }

            $recvUi   = (float)($recieved[$i] ?? $qty[$i] ?? 0);
            $recvBase = ($unitModel->operator == '*')
                ? ($recvUi * $unitModel->operation_value)
                : ($recvUi / max(1, $unitModel->operation_value));
            ...
            if (!empty($batch_no[$i])) {
                ...
                if ($batch) {
                    $batch->qty += $recvBase;
                    if (!empty($expired_date[$i])) $batch->expired_date = $expired_date[$i];
                    $batch->save();
                } else {
            ...
            // variant/warehouse
            $variant_id = null; $pw = null; $price = null;
            if ($product->is_variant) {
                $pv = ProductVariant::select('id', 'variant_id', 'qty')
                    ->FindExactProductWithCode($pid, $product_code[$i] ?? null)->first();
                if (!$pv) { DB::rollBack(); return back()->with('not_permitted', __('db.Invalid variant for product.')); }
                ...
                $pv->qty += $recvBase; $pv->save();
            } else {
                if ($product_batch_id) {
                    $priced = Product_Warehouse::where([
                        ['product_id', $pid], ['warehouse_id', $data['warehouse_id']],
                    ])->whereNotNull('price')->select('price')->first();
                    if ($priced) $price = $priced->price;

                    $pw = Product_Warehouse::where([
                        ['product_id', $pid], ['product_batch_id', $product_batch_id], ['warehouse_id', $data['warehouse_id']]
                    ])->first();
                } else {
                    $pw = Product_Warehouse::where([
                        ['product_id', $pid], ['warehouse_id', $data['warehouse_id']]
            ...
            $product->qty += $recvBase; $product->save();

            if ($pw) {
                $pw->qty += $recvBase;
                if ($price && !$pw->price) $pw->price = $price;
                $pw->save();
            } else {
                $pw = new Product_Warehouse();
                $pw->product_id = $pid;
                $pw->product_batch_id = $product_batch_id;
                if ($variant_id) $pw->variant_id = $variant_id;
                $pw->warehouse_id = $data['warehouse_id'];
                $pw->qty = $recvBase;
                if ($price) $pw->price = $price;
                $pw->save();
            }

            // IMEI duplicate checks
            if (!empty($imei_number[$i])) {
                $imeis = array_filter(array_map('trim', explode(',', $imei_number[$i])));
                if (count($imeis) !== count(array_unique($imeis))) {
                    DB::rollBack(); return back()->with('not_permitted', __('db.Duplicate IMEI not allowed!'));
                }
                foreach ($imeis as $imei) {
                    if ($this->isImeiExist($imei, $pid)) {
                        DB::rollBack(); return back()->with('not_permitted', __('db.Duplicate IMEI not allowed!'));
                    }
                }
                if ($pw->imei_number) $pw->imei_number .= ',' . implode(',', $imeis);
                else $pw->imei_number = implode(',', $imeis);
                $pw->save();
            }
            ...
            ProductPurchase::create([
                ...
                'qty'               => (float)($qty[$i] ?? 0),
                'recieved'          => $recvUi,
                'purchase_unit_id'  => $unitModel->id,
                'net_unit_cost'     => (float)($net_unit_cost[$i] ?? 0),
                'discount'          => (float)($discount[$i] ?? 0),
                'tax_rate'          => (float)($tax_rate[$i] ?? 0),
                'tax'               => (float)($tax[$i] ?? 0),
                'total'             => (float)($total[$i] ?? 0),
                'imei_number'       => $imei_number[$i] ?? null,
                'moq'               => $moq[$i] ?? null,
                'ship_term'         => $ship_term[$i] ?? null,
                'ship_cost'         => (float)($ship_cost[$i] ?? 0),
                'supplier_id'       => !empty($supplier_ids[$i]) ? (int)$supplier_ids[$i] : null,
                'ets_date'          => $normalizeDate($ets_date[$i] ?? null),
                'eta_date'          => $normalizeDate($eta_date[$i] ?? null),
                'etd_date'          => $etd_date[$i] ?? "",
        ...
        foreach ($newHeader as $k => $v) {
            if (!array_key_exists($k, $origHeader)) continue;
            if ((string)$origHeader[$k] === (string)$v) continue;
        ...
        // LINES DIFF
        $newLines = ProductPurchase::where('purchase_id', $id)->get();
        $newMap = [];
        foreach ($newLines as $pp) {
            $key = $pp->product_id . ':' . ((int)$pp->variant_id);
            $newMap[$key] = $snapLine($pp);
        }
        ...
        // modified + removed
        foreach ($origMap as $key => $old) {
            if (!isset($newMap[$key])) {
                // removed
                $linesDiff['removed'][] = [
                    'product_id' => $old['product_id'],
                    'variant_id' => $old['variant_id'],
                    'product'    => optional(Product::find($old['product_id']))->name,
                ];
                continue;
            }
            $new = $newMap[$key];

            // keys to compare (IMPORTANT: only those you care)
            $compareKeys = [
                'qty','recieved','purchase_unit','net_unit_cost','discount','tax_rate','tax',
                'total','supplier_name','imei_number','ets_date','eta_date','etd_date','moq',
                'ship_term','ship_cost','batch_no','lot_no','expired_date'
            ];

            $changed = [];
            foreach ($compareKeys as $ck) {
                $oldVal = $old[$ck] ?? null;
                $newVal = $new[$ck] ?? null;
                if ((string)$oldVal !== (string)$newVal && $meaningful($newVal)) {
                    $changed[$ck] = $newVal;   // <-- ONLY changed field gets logged
                }
            }
            ...
            if (!empty($changed)) {
                $linesDiff['modified'][] = [
                    ...
                    'product'    => optional(Product::find($new['product_id']))->name,
                    'changes'    => $changed,
                ];
            }
        }
        ...
        foreach ($newMap as $key => $new) {
            if (!isset($origMap[$key])) {
                // concise add: only meaningful fields (not everything)
                $row = [
                    'product_id' => $new['product_id'],
                    'variant_id' => $new['variant_id'],
                    'product'    => optional(Product::find($new['product_id']))->name,
                ];
                foreach (['qty','recieved','purchase_unit','net_unit_cost','tax_rate','total','supplier_name','batch_no','lot_no','expired_date','ets_date','eta_date','ship_cost'] as $k2) {
                    $val2 = $new[$k2] ?? null;
                    if ($meaningful($val2)) $row[$k2] = $val2;
                }
                $linesDiff['added'][] = $row;
            }
        }

        // ---------------- Write ONE log row only when something changed ----------------
        if (!empty($headerDiff) || !empty($linesDiff['modified']) || !empty($linesDiff['added']) || !empty($linesDiff['removed'])) {
            DB::table('product_purchase_logs')->insert([
                'purchase_id' => $purchase->id,
                'user_id'     => Auth::id(),
                'notes'       => json_encode([
   ...
   private function generatePoNumber()
{
    ...
    // purchases ki last row nikal lo
    $lastPo = DB::table('purchases')
        ->orderBy('id', 'desc')
        ->first();

    if ($lastPo && !empty($lastPo->system_po_no)) {
        // last PO number ke last 4 digits nikal lo
        $lastSuffix = (int) substr($lastPo->system_po_no, -4);
        $next = $lastSuffix + 1;
    } else {
        // agar koi record nahi hai to 1 se start
        $next = 1;
    }

    // 4 digits banane ke liye pad karna
