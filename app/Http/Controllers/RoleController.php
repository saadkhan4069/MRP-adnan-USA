...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('payment_gateway_setting')){
            $permission = Permission::firstOrCreate(['name' => 'payment_gateway_setting']);
            if(!$role->hasPermissionTo('payment_gateway_setting')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('payment_gateway_setting');
        ...
        if($request->has('barcode_setting')){
            $permission = Permission::firstOrCreate(['name' => 'barcode_setting']);
            if(!$role->hasPermissionTo('barcode_setting')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('barcode_setting');

        if($request->has('language_setting')){
            $permission = Permission::firstOrCreate(['name' => 'language_setting']);
            if(!$role->hasPermissionTo('language_setting')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('language_setting');
        ...
        if($request->has('invoice_setting')){
            $permission = Permission::firstOrCreate(['name' => 'invoice_setting']);
            if(!$role->hasPermissionTo('invoice_setting')){
                $role->givePermissionTo($permission);
            }
        }
        else {
            $permission = Permission::where('name', 'invoice_setting')->first();
            if($permission)
                $role->revokePermissionTo('invoice_setting');
        }

        if($request->has('invoice_create_edit_delete')){
            $permission = Permission::firstOrCreate(['name' => 'invoice_create_edit_delete']);
            if(!$role->hasPermissionTo('invoice_create_edit_delete')){
                $role->givePermissionTo($permission);
            }
        }
        else {
            $permission = Permission::where('name', 'invoice_create_edit_delete')->first();
            if($permission)
                $role->revokePermissionTo('invoice_create_edit_delete');
        }
        ...
        if($request->has('addons')) {
            $permission = Permission::firstOrCreate(['name' => 'addons']);
            if(!$role->hasPermissionTo('addons')) {
                $role->givePermissionTo($permission);
            }
        }
        else {
            $permission = Permission::where('name', 'addons')->first();
            if($permission)
