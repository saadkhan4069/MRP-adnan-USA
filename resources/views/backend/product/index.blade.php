...
<section>
    <div class="container-fluid">
        @if(in_array("products-add", $all_permission))
            <a href="{{route('products.create')}}" class="btn btn-info add-product-btn"><i class="dripicons-plus"></i> {{__('db.add_product')}}</a>
            <a href="#" data-toggle="modal" data-target="#importProduct" class="btn btn-primary add-product-btn"><i class="dripicons-copy"></i> {{__('db.import_product')}}</a>
        ...
        @endif
        <div class="card mt-3">
            <h3 class="text-center mt-3">{{__('db.Filter Products')}}</h3>
            <div class="card-body">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p class="italic"><small>{{__('db.The field labels marked with * are required input fields')}}.</small></p>
           <p>{{__('db.The correct column order is')}} (image, name*, code*, type*, brand, category*, unit_code*, cost*, price*, product_details, variant_name, item_code, additional_price) {{__('db.and you must follow this')}}.</p>
...
<script>
    ...
    function productDetails(product, imagedata) {
        ...
        htmltext = '<p><strong>{{__("db.Type")}}: </strong>'+product[0]+
            '</p><p><strong>{{__("db.name")}}: </strong>'+product[1]+
            '</p><p><strong>{{__("db.Code")}}: </strong>'+product[2]+
            '</p><p><strong>{{__("db.Brand")}}: </strong>'+product[3]+
            '</p><p><strong>{{__("db.category")}}: </strong>'+product[4]+
            '</p><p><strong>{{__("db.Quantity")}}: </strong>'+product[17]+
            '</p><p><strong>{{__("db.Unit")}}: </strong>'+product[5]+
            '</p><p><strong>{{__("db.Cost")}}: </strong>'+product[6]+
            '</p><p><strong>{{__("db.Price")}}: </strong>'+product[7]+
            '</p><p><strong>{{__("db.Tax")}}: </strong>'+product[8]+
            '</p><p><strong>{{__("db.Tax Method")}} : </strong>'+product[9]+
            '</p><p><strong>{{__("db.Alert Quantity")}} : </strong>'+product[10]+
            '</p><p><strong>{{__("db.Product Details")}}: </strong></p>'+product[11];
        ...
        if(product[18]) {
            ...
            if(product_image.length > 1) {
                ...
                for (var i = 0; i < product_image.length; i++) {
                    if(!i)
                        slidertext += '<div class="carousel-item active"><img src="images/product/'+product_image[i]+'" width="100%"></div>';
                    else
                        slidertext += '<div class="carousel-item"><img src="images/product/'+product_image[i]+'"  width="100%"></div>';
                }
        ...
        if(product[0] == 'combo') {
            ...
            var newBody = $("<tbody>");
            var newRow = $("<tr>");
            newRow.append('<th>{{__("db.product")}}</th><th>{{__("db.Quantity")}}</th><th>{{__("db.Price")}}</th>');
