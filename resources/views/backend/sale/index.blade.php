...
#sale-details .btn-default:hover {
...
<section>
    <div class="container-fluid">
        @if(in_array("sales-add", $all_permission))
            <a href="{{route('sales.create')}}" class="btn btn-info add-sale-btn"><i class="dripicons-plus"></i> {{__('db.Add Sale')}}</a>&nbsp;
            <a href="{{url('sales/sale_by_csv')}}" class="btn btn-primary add-sale-btn"><i class="dripicons-copy"></i> {{__('db.Import Sale')}}</a>
        @endif
        <div class="card mt-3">
            <h3 class="text-center mt-3">{{__('db.Filter Sales')}}</h3>
            <div class="card-body">
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            <label><strong>{{__('db.Sale Status')}}</strong></label>
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            <label><strong>{{__('db.Payment Method')}}</strong></label>
                    ...
                    <div class="col-md-2 @if(!in_array('ecommerce',explode(',',$general_setting->modules))) d-none @endif">
                        <div class="form-group">
                            <label><strong>{{__('db.Sale Type')}}</strong></label>
        ...
        <div class="modal-content">
            <div class="container mt-3 pb-2 ">
                <div class="row">
                    ...
                    <h3 id="exampleModalLabel" class="modal-title text-center container-fluid" >{{__('Sale Details')}}</h3>
            ...
            <br>
            <table class="table table-bordered product-sale-list product-quotation-list_design">
                <thead>
                    <th>#</th>
                    <th>{{__('db.product')}}</th>
...
function sweetConfirm({title='Are you sure?', text='', confirmText='Yes', cancelText='Cancel', icon='warning'}={}) {
  return Swal.fire({
    ...
    var columns = [{"data": "key"}, {"data": "date"}, {"data": "reference_no"}, {"data": "biller"}, {"data": "customer"}, {"data": "warehouse_name"}, {"data": "sale_status"}, {"data": "payment_status"},{"data": "payment_method"},{"data": "delivery_status"}, {"data": "grand_total"}, {"data": "returned_amount"}, {"data": "paid_amount"}, {"data": "due"}];
    ...
    $(document).on("click", "table.sale-list tbody .add-payment", function() {
        ...
        var balance = $('table.sale-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('td:nth-child(14)').text();
    ...
    $('#sale-table').DataTable( {
        ...
        "footerCallback": function(row, data, start, end, display) {
            ...
            // Calculate the total amounts grouped by payment methods
            var paymentTotals = {};
    
            data.forEach(function(row) {
                var payments = row['payment_method']; // Adjust this column key based on your data structure
                if (payments) {
                    payments.split(', ').forEach(function(payment) {
                        var match = payment.match(/(.+)\(([\d.,]+)\)/); // Extract payment method and amount
                        if (match) {
                            var method = match[1];
            ...
            // Add the content to the footer (adjust column index as needed)
    ...
    function datatable_sum(dt_selector, is_calling_first) {
        if (dt_selector.rows( '.selected' ).any() && is_calling_first) {
            ...
            $( dt_selector.column( 13 ).footer() ).html(dt_selector.cells( rows, 12, { page: 'current' } ).data().sum().toFixed({{$general_setting->decimal}}));
        }
        else {
            $( dt_selector.column( 10).footer() ).html(dt_selector.cells( rows, 10, { page: 'current' } ).data().sum().toFixed({{$general_setting->decimal}}));
            $( dt_selector.column( 11 ).footer() ).html(dt_selector.cells( rows, 11, { page: 'current' } ).data().sum().toFixed({{$general_setting->decimal}}));
    ...
    function saleDetails(sale){
        ...
        var htmltext = `
         <div class="row">
<h3 style="text-align:center;margin:0 auto;width:51%;margin-bottom:17px;margin-top:-18px;font-weight:700;"># `+sale[38]+`</h3>
          <div class="col-md-6">
           <strong style="color:#9a191c">Bill To:</strong>
        ...
        `</div>
            ...
            <div class="col-md-5">
             ...
             <strong>{{__("db.date")}}: </strong>`+sale[0]+
            `<br><strong>{{__("db.reference")}}: </strong>`+sale[1]+
            `<br><strong>{{__("db.Warehouse")}}: </strong>`+sale[27]+
            `<br><strong>{{__("db.Sale Status")}}: </strong>`
             if(sale[2] == "Pending") {
                     htmltext += `<div class="badge badge-danger">`+sale[2]+`</div>`;
                }
                else{
                     htmltext += `<div class="badge badge-success">`+sale[2]+`</div>`;
                }
          
        if(sale[30])
            htmltext += `<br><strong>{{__("db.Attach Document")}}: </strong><a href="documents/sale/`+sale[30]+`">Download</a>`;
        ...
        <div class="float-left">
         <strong style="color:#9a191c">Ship From:</strong>
        <br>
        <strong> Name:</strong>`+sale[33]+`
        <br>
        <strong> Company:</strong>`+sale[34]+`
        <br>
        <strong> Phone:</strong>`+sale[35]+`
        <br>
       <strong> Email:</strong> `+sale[36]+`
        <br>
        <strong> Address:</strong>`+sale[37]+`
        </div>
        </div>

         <div class="col-md-6">
        <div class="float-right">
         <strong style="color:#9a191c">Ship To:</strong>
        <br>
       <strong> Name:</strong>`+sale[9]+`
        <br>
       <strong> Phone:</strong>`+sale[10]+`
        <br>
       <strong> Address:</strong> `+sale[11]+`
        <br>
       <strong> City:</strong> `+sale[12]+`
        </div>
        </div>
    ...
    htmltext  += `
        ...
        if (Array.isArray(data[11])) {
            ...
            console.warn("data[11] is not an array");
...
$('#sale-content').html(htmltext);
            ...
            var moq = data[14];
            var lead_time = data[15];
            var ship_term = data[12];
            var ship_cost = data[13];
            ...
            var newRow = $("<tr>");
            cols = '';
            cols += '<td colspan=3><strong style="float: left;">{{__("db.Total")}}:</strong></td>';
            ...
            var newRow = $("<tr>");
            cols = '';
            cols += '<td colspan=9><strong style="float: left;">{{__("db.Order Tax")}}:</strong></td>';
            ...
            var newRow = $("<tr>");
            cols = '';
            cols += '<td colspan=9><strong style="float: left;">{{__("db.Order Discount")}}:</strong></td>';
            cols += '<td>' + sale[19] + '</td>';
            newRow.append(cols);
            newBody.append(newRow);
            if(sale[28]) {
                var newRow = $("<tr>");
                cols = '';
                cols += '<td colspan=9><strong style="float: left;">{{__("db.Coupon Discount")}} ['+sale[28]+']:</strong></td>';
                cols += '<td>' + sale[29] + '</td>';
                newRow.append(cols);
                newBody.append(newRow);
            }
            ...
            var newRow = $("<tr>");
            cols = '';
            cols += '<td colspan=9><strong style="float: left;">{{__("db.Shipping Cost")}}:</strong></td>';
            cols += '<td>' + sale[20] + '</td>';
            newRow.append(cols);
            newBody.append(newRow);
            ...
            var newRow = $("<tr>");
            cols = '';
            cols += '<td colspan=9><strong style="float: left;">{{__("db.grand total")}}:</strong></td>';
            cols += '<td>' + sale[21] + '</td>';
            newRow.append(cols);
            newBody.append(newRow);
            ...
            var newRow = $("<tr>");
            cols = '';
            cols += '<td colspan=9><strong style="float: left;">{{__("db.Paid Amount")}}:</strong></td>';
            cols += '<td>' + sale[22] + '</td>';
            newRow.append(cols);
            newBody.append(newRow);
            ...
            var newRow = $("<tr>");
            cols = '';
            cols += '<td colspan=9><strong style="float: left;">{{__("db.Due")}}:</strong></td>';
...
$(document).on('click', '.generate-pdf', function () {
    ...
    var url = 'sale/pdf/' + supplierId + '/' + sale_id;
