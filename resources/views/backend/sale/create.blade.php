...
<section id="pos-layout" class="forms hidden-print">
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="row">
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      <label>{{__('Ship To')}} * <small>Customer</small></label>
                        ...
                        @if($customer_active)
                          <select required name="customer_id" id="customer_id" class="selectpicker form-control" data-live-search="true" title="Select customer..." style="width: 100px">
                            @foreach($lims_customer_list as $customer)
                              ...
                              @endphp
                              <option value="{{$customer->id}}" data-type="{{$customer->type}}">{{$customer->name . ' (' . $customer->phone_number . ')'}}</option>
                            @endforeach
                          </select>
                  ...
                  @else
                    <div class="col-md-4">
                      <div class="form-group">
                        <label>{{__('Ship From')}} * <small>Warehouse</small></label>
                ...
                <div class="row">
                  ...
                  <div class="col-md-2"><div class="form-group"><input type="hidden" name="total_price" /></div></div>
                  <div class="col-md-2">
                    <div class="form-group">
                    ...
                    <div class="col-md-4">
                      <div class="form-group">
                        ...
                        <select name="paid_by_id[]" class="form-control">
                          @if(in_array("cash",$options))     <option value="1">{{ __('db.Cash') }}</option>@endif
                          @if(in_array("gift_card",$options))<option value="2">{{ __('db.Gift Card') }}</option>@endif
                          @if(in_array("card",$options))     <option value="3">{{ __('db.Credit Card') }}</option>@endif
                          @if(in_array("cheque",$options))   <option value="4">{{ __('db.Cheque') }}</option>@endif
                          ...
                          @endif
                          @if(in_array("deposit",$options))  <option value="6">{{ __('db.Deposit') }}</option>@endif
                          @if($lims_reward_point_setting_data && $lims_reward_point_setting_data->is_active)
                            <option value="7">{{ __('db.Points') }}</option>
                          @endif
                          @if(in_array("pesapal",$options))  <option value="8">{{ __('db.Pesapal') }}</option>@endif
  ...
  <div class="container-fluid">
    <table class="table table-bordered table-condensed totals">
      ...
      <td><strong>{{__('db.Order Discount')}}</strong> <span class="pull-right" id="order_discount">{{number_format(0, $general_setting->decimal, '.', '')}}</span></td>
              ...
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <div class="input-group">
                    <select class="form-control selectpicker" name="price_option" class="price-option"></select>
                  </div>
                </div>
              </div>
              <div class="col-md-4 form-group">
  ...
  <!-- ... the rest of modals remain exactly as in your original (no batch fields there) ... -->
...
function sweetConfirm({title='Are you sure?', text='', confirmText='Yes', cancelText='Cancel', icon='warning'}={}) {
  return Swal.fire({ title, text, icon, showCancelButton: true, confirmButtonText: confirmText, cancelButtonText: cancelText, reverseButtons: true, focusCancel: true }).then(r => r.isConfirmed);
}
</script>
...
function getProduct(warehouse_id){
  $.get('getproduct/' + warehouse_id, function(data) {
    ...
    // batch_no, product_batch_id, expired_date removed from UI — ignore indices [8],[9],[10]
    is_embeded   = data[11];
    imei_number  = data[12];
...
lims_productcodeSearch.autocomplete({
  source: function(request, response) {
    ...
    response($.grep(lims_product_array, function(item) { return matcher.test(item); }));
  },
...
$("table.order-list").on("click", ".edit-product", function() {
   ...
   var ltdate =  $(".lt-date"+$(this).attr('data-no')).val()
...
$('button[name="update_btn"]').on("click", function() {
    ...
    var lt_date = $('input[name="lt_date"]').val();
...
// NOTE: Batch/Lot/Expiry handlers removed
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(flag){
          ...
          cols += '<td><input type="text" class="form-control batch_no" name="batch_no[]" value=""  /></td>';
			let supplierOptions = `
			<td>
          ...
          if(data[18])
		    ...
		    cols += '<input type="hidden" class="lt-date'+flag2+'" name="lt_date[]" />';
  ...
  if(is_imei[rowindex]) {
    ...
    if(imeiNumbers.length) {
      ...
      for (var i = 0; i < imeiArrays.length; i++) {
        htmlText += `<tr>
          <td><input type="text" class="form-control imei-numbers" name="imei_numbers[]" value="`+imeiArrays[i]+`" /></td>
          <td><button type="button" class="imei-del btn btn-sm btn-danger">X</button></td>
        </tr>`;
      }
...
function checkQuantity(sale_qty, flag) {
  ...
  if(without_stock == 'no') {
    if(product_type[pos] == 'standard' || product_type[pos] == 'combo'){
      ...
      if(operator[0] == '*') total_qty = sale_qty * operation_value[0];
      else if(operator[0] == '/') total_qty = sale_qty / operation_value[0];
...
function unitConversion() {
  var row_unit_operator = unit_operator[rowindex].slice(0, unit_operator[rowindex].indexOf(","));
  var row_unit_operation_value = unit_operation_value[rowindex].slice(0, unit_operation_value[rowindex].indexOf(","));
  if (row_unit_operator == '*') row_product_price = product_price[rowindex] * row_unit_operation_value;
  else row_product_price = product_price[rowindex] / row_unit_operation_value;
}

function calculateTotal() {
  // qty
  var total_qty = 0;
  $(".qty").each(function() { total_qty += ($(this).val() == '') ? 0 : parseFloat($(this).val()); });
  $("#total-qty").text(total_qty);
  $('input[name="total_qty"]').val(total_qty);

  // discount
  var total_discount = 0;
  $(".discount").each(function() { total_discount += parseFloat($(this).text()); });
  ...
  // tax
  var total_tax = 0;
  $(".tax").each(function() { total_tax += parseFloat($(this).text()); });
  ...
  // subtotal
  var total = 0;
  $(".sub-total").each(function() { total += parseFloat($(this).text()); });
...
function calculateGrandTotal() {
  ...
  if(order_discount_type == 'Flat') {
    order_discount = currencyChange ? parseFloat(order_discount_value*currency['exchange_rate']) : parseFloat(order_discount_value);
  } else {
...
$('select[name="order_discount_type"]').on("change", function() { calculateGrandTotal(); });
$('input[name="order_discount_value"]').on("input", function() { calculateGrandTotal(); });
$('input[name="shipping_cost"]').on("input", function() { calculateGrandTotal(); });
$('select[name="order_tax_rate"]').on("change", function() { calculateGrandTotal(); });
...
$('input[name="paid_amount[]"]').on("input", function() {
  ...
  if(id == 2){
    ...
    if($(this).val() > balance) alert('Amount exceeds card balance! Gift Card balance: '+ balance);
  }
  else if(id == 6){
    if( $('input[name="paid_amount[]"]').val() > deposit[$('#customer_id').val()] )
      alert('Amount exceeds customer deposit! Customer deposit : '+ deposit[$('#customer_id').val()]);
...
$("#submit-button").on("click", function() { $('.payment-form').submit(); });
...
$(document).on('submit', '.payment-form', function(e) {
  ...
  if (customer_type === 'walkin' && current_payment_status !== 4) {
    ...
    $("#paid-amount").prop('disabled',false);
    // Removed: $(".batch-no").prop('disabled', false);
...
location.href = response;
      ...
      error: function(xhr) {
        ...
        toastWarn(xhr.responseJSON.message)
      }
    });
  }
});
