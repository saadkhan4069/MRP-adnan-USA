...
?>
<td>{{$product_data->name}} <button type="button" class="edit-product btn btn-link" data-toggle="modal" data-target="#editModal" data-no="{{ $flag2 }}"> <i class="dripicons-document-edit"></i></button> <input type="hidden" class="product-type" value="{{$product_data->type}}" /></td>
<td>{{$product_data->code}}</td>

 <td><input type="number" class="form-control qty" name="qty[]" value="{{$product_sale->qty}}" step="any" required/></td>

 <td><input type="text" class="form-control batch_no" name="batch_no[]" value="{{$product_sale->batch_no}}"  /></td>
...
@if(isset($product_sale['supplier_id']) && $product_sale['supplier_id'] == $supplier->id) selected @endif>
{{ $supplier->company_name . ' (' . $supplier->name . ')' }}   
</option>
@endforeach
</select>
</td>
<td class="ship_costt">{{ number_format((float)$product_sale->ship_cost, $general_setting->decimal, '.', '')}}</td>
...
<input type="hidden" class="eta-date{{ $flag2 }}" name="eta_date[]" value="{{ date('d-m-Y', strtotime($product_sale->eta_date)) }}" />

<input type="hidden" class="ets-date{{ $flag2 }}" name="ets_date[]" value="{{ date('d-m-Y', strtotime($product_sale->ets_date)) }}" />

<input type="hidden" class="etd-date{{ $flag2 }}" name="lt_date[]" value="{{ $product_sale->lt_date }}" />

<input type="hidden" class="moq{{ $flag2 }}" name="moq[]" value="{{ $product_sale->moq }}" />

<input type="hidden" class="ship_cost{{ $flag2 }}" name="ship_cost[]" value="{{ $product_sale->ship_cost }}" />

<input type="hidden" class="ship_term{{ $flag2 }}" name="ship_term[]" value="{{ $product_sale->ship_term }}" />
...
<div class="container-fluid mt-3">
  <div class="card">
    ...
    <div class="card-body p-0">
      @if(isset($lims_saleLogs) && $lims_saleLogs->count())
        <div class="table-responsive">
          <table class="table table-bordered mb-0">
            ...
            <tbody>
              @foreach($lims_saleLogs as $log)
                @php
                  // notes ko array bana lo (TEXT me JSON store kia tha)
                  $n = is_array($log->notes) ? $log->notes : json_decode($log->notes, true) ?? [];

                  // labels ko pretty text me map
                  $labelMap = [
                    ...
                    'order_discount_value'=> 'Order discount',
                    ...
                    'sale_note'           => 'Note',
                    'staff_note'          => 'Comments',
                    // line-item keys (agar aa rahe hon)
                    'product_id'          => 'Product',
                    'product_code'        => 'Code',
                    'qty'                 => 'Quantity',
                    'batch_no'            => 'Batch No',
                  ...
                  // IDs ko name me convert karne ka helper
                  $resolve = function($key, $value) use ($warehouseNames,$customerNames,$billerNames){
                      if($key==='warehouse_id' && isset($warehouseNames[$value])) return $warehouseNames[$value];
                      if($key==='customer_id'  && isset($customerNames[$value]))  return $customerNames[$value];
                      if($key==='biller_id'    && isset($billerNames[$value]))    return $billerNames[$value];
                      return $value;
                  };
                @endphp
                <tr>
                  ...
                  <td>
                    <ul class="mb-0 pl-3">
                      @foreach($n as $k => $v)
                        @php $label = $labelMap[$k] ?? ucwords(str_replace('_',' ',$k)); @endphp

                        {{-- arrays (hum store me arrays ko {"count":x,"first":[...]} jaisa rakh rahe they) --}}
                        @if(is_array($v))
                          @php
                            $valToShow = isset($v['first'])
                                          ? (is_array($v['first']) ? implode(', ', $v['first']) : $v['first'])
                                          : implode(', ', $v);
                          @endphp
                          @if($valToShow !== '')
                            <li><strong>{{ $label }}:</strong> {{ $valToShow }}</li>
                          @endif
                        @else
                          @if($v !== null && $v !== '')
                            <li><strong>{{ $label }}:</strong> {{ $resolve($k,$v) }}</li>
                          @endif
                        @endif
                      @endforeach
                    </ul>
      ...
      @else
        <p class="p-3 mb-0"><em>No logs yet.</em></p>
      @endif
    </div>
  </div>
</div>
...
$("table.order-list").on("click", ".edit-product", function() {
    $('button[name="update_btn"]').attr("id",$(this).attr('data-no'));
        ...
        if (etddate !=""){ $('input[name="lt_date"]').val(etddate) }
...
$('button[name="update_btn"]').on("click", function() {
    ...
    var etddate = $('input[name="lt_date"]').val();
...
cols += '<td>' + data[0] + '<button type="button" class="edit-product btn btn-link"  data-no='+flag2+' data-toggle="modal" data-target="#editModal"> <i class="dripicons-document-edit"></i></button></td>';
cols += '<td>' + data[1] + '</td>';
...
cols += '<input type="hidden" class="etd-date'+flag2+'" name="lt_date[]" />';
