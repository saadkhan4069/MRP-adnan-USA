    ...
    <div style="max-width:290px;margin:0 auto">
        ...
        <div id="receipt-data">
            @if (isset($show->show_warehouse_info) && $show->show_warehouse_info == 1)
                <div class="centered">
                    ...
                    <img src="{{ $invoice_settings->company_logo ? $invoice_settings->company_logo : url('logo', $general_setting->site_logo) }}"
                        height="{{ $invoice_settings->logo_height ?? 42 }}" width="{{ $invoice_settings->logo_width ?? 50 }}" style="margin:10px 0;">
                @else
                    <img src="{{ $invoice_settings->company_logo ? $invoice_settings->company_logo : url('logo', $general_setting->site_logo) }}"
                        height="42" width="50">
                @endif
            ...
            <p>{{ __('db.date') }}:
                ...
                @endif                {{-- {{ date($general_setting->date_format, strtotime($lims_sale_data->created_at->toDateString())) }} --}}
                <br>
                ...
                <tbody>
                    <div style="text-align: right;">
                        @if (isset($show->show_biller_info) && $show->show_biller_info == 1)
                            <h3>{{ $lims_bill_by['name'] }} - ({{ $lims_bill_by['user_name'] }})</h3>
                        @endif
                    </div>
                        ...
                        @foreach ($lims_payment_data as $payment_data)
                            <tr style="background-color:#ddd;">
                                ...
                                <td style="padding: 5px;width:40%">{{ __('db.Amount') }}:
                                    {{ number_format((float) $payment_data->amount, $general_setting->decimal, '.', ',') }}
                                </td>
