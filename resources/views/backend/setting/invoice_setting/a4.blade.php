    ...
    <table style="width: 100%;border-collapse: collapse;">
        <tr>
            ...
            <td style="width:30%; text-align: middle; vertical-align: top;">
                @if (isset($show->active_logo_height_width) && $show->active_logo_height_width == 1)
                    <img src="{{ $invoice_settings->company_logo ? $invoice_settings->company_logo : url('logo', $general_setting->site_logo) }}"
                        height="{{ $invoice_settings->logo_height ?? 80 }}"
                        width="{{ $invoice_settings->logo_width ?? 120 }}">
                @else
                    <img src="{{ $invoice_settings->company_logo ? $invoice_settings->company_logo : url('logo', $general_setting->site_logo) }}"
                        height="80" width="120">
                @endif
    ...
    <table dir="@if (Config::get('app.locale') == 'ar' || $general_setting->is_rtl) {{ 'rtl' }} @endif"
        ...
        @foreach ($lims_product_sale_data as $key => $product_sale_data)
            ...
            <tr>
                ...
                <td style="border:1px solid #222;padding:1px 3px;text-align:center">
                    {{ number_format($product_sale_data->net_unit_price, $general_setting->decimal, '.', ',') }}
                ...
                <td style="border:1px solid #222;padding:1px 3px;text-align:center">
                    {{ number_format($total, $general_setting->decimal, '.', ',') }}</td>
                <td style="border:1px solid #222;padding:1px 3px;text-align:center">
                    {{ number_format($product_sale_data->tax, $general_setting->decimal, '.', ',') }}</td>
                <td
        ...
        @endforeach
        <tr>
            <td colspan="3" rowspan="@if ($general_setting->invoice_format == 'gst' && $general_setting->state == 2) 5 @else 4 @endif"
                style="border:1px solid #222;padding:1px 3px;text-align: center; vertical-align: top;">
                @if (isset($show->show_payment_note) && $show->show_payment_note == 1)
                    {{ __('db.Note') }}<br>{{ $lims_sale_data->sale_note }}
                @endif
            </td>
