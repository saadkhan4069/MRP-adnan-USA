...
<section>
    <div class="container-fluid">
        <div class="md-12">
            <button class="btn btn-info" data-toggle="modal" data-target="#createModal"><i class="dripicons-plus"></i> {{__('db.Add Attendance')}} </button>
        </div>
        <div class="md-12">
            <!-- Import CSV File (Device) -->
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{__('db.Import CSV file (Device)')}}</h3>
                </div>
                <div class="card-body">
                        ...
                        <div class="form-group">
                            <fieldset class="form-group">
                                <div class="row">
                                    ...
                                    <div class="md-4">
                                        <label for="logo">{{__('db.Upload File')}}</label>
                                        <input type="file" class="form-control-file" name="file"
                                                accept=".xlsx, .xls, .csv">
                                    </div>
                                    <div class="md-4">
                                        <button name="import_form" type="submit" class="btn btn-primary"><i
                                                class="fa fa fa-check-square-o"></i> {{__('db.Save')}}
                                        </button>
                                    </div>
                                 </div>
            ...
            <tbody>
                ...
                <tr data-date="{{$attendance['date']}}" data-employee_id="{{$attendance['employee_id']}}">
                    ...
                    @if($attendance['status'])
                        <td><div class="badge badge-success">{{__('db.Present')}}</div></td>
                    @else()
                        <td><div class="badge badge-danger">{{__('db.Late')}}</div></td>
                    @endif
                    <td>{{ $attendance['user_name'] }}</td>
                    <td>
                        <div class="btn-group">
                            {{ Form::open(['route' => ['attendances.delete', [$attendance['date'], $attendance['employee_id']]], 'method' => 'post'] ) }}
                            <button type="submit" class="btn btn-sm btn-danger" onclick="return confirmDelete()" title="{{__('db.delete')}}"><i class="dripicons-trash"></i></button>
                            {{ Form::close() }}
                        </div>
                    </td>
                </tr>
