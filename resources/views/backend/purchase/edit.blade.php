...
<input type="text" name="ship_term" class="form-control ship_term" placeholder="{{__('Pre-Paid OR Post-Paid')}}" required />
  ...
  // ====== Helpers ======
  const DEC = {{$general_setting->decimal}};
  const f = (n) => Number.isFinite(n) ? n.toFixed(DEC) : (0).toFixed(DEC);
  const num = (v) => {
    if (v === null || v === undefined) return 0;
    if (typeof v === 'string') {
      v = v.trim();
      if (v === '' || v.toLowerCase() === 'nan') return 0;
    }
  ...
  // Header summary initial
  $('#item').text($('input[name="item"]').val() + '(' + $('input[name="total_qty"]').val() + ')');
  $('#subtotal').text(f(num($('input[name="total_cost"]').val())));
  $('#order_tax').text(f(num($('input[name="order_tax"]').val())));
  if(!$('input[name="order_discount"]').val()) $('input[name="order_discount"]').val(f(0));
  $('#order_discount').text(f(num($('input[name="order_discount"]').val())));

  // shipping cost = sum of per-line ship_cost[]
  (function initShippingHeader(){
    let t = 0;
    $('input[name="ship_cost[]"]').each(function(){ t += num($(this).val()); });
    $('#shipping_cost').text(f(t));
  })();
  $('#grand_total').text(f(num($('input[name="grand_total"]').val())));

  // Show recieved col if needed
  if ($('select[name="status"]').val() == 2) $(".recieved-product-qty").removeClass("d-none");

  // ====== Build arrays from existing DOM ======
  var rownumber = $('table.order-list tbody tr:last').index();
  for (rowindex = 0; rowindex <= rownumber; rowindex++) {
    product_cost.push(num($('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.product-cost').val()));

    // per-unit discount from total discount cell / qty
    const total_disc_txt = $('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.discount').text();
    const qty_val = num($('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.qty').val());
    product_discount.push(qty_val ? (num(total_disc_txt)/qty_val).toFixed(DEC) : f(0));

    tax_rate.push(num($('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.tax-rate').val()));
    tax_name.push($('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.tax-name').val());
    tax_method.push(num($('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.tax-method').val()));
  ...
  $("table.order-list").on("click", ".edit-product", function() {
    ...
    // prefill ship/eta data
    $('input[name="ets_date"]').val($(".ets-date"+id).val() || '');
    $('input[name="eta_date"]').val($(".eta-date"+id).val() || '');
    $('input[name="etd_date"]').val($(".etd-date"+id).val() || '');
    $('input[name="moq"]').val($(".moq"+id).val() || '');
    $('input[name="ship_term"]').val($(".ship_term"+id).val() || '');
    $('input[name="ship_cost"]').val($(".ship_cost"+id).val() || '');
    ...
    // IMEI section
    if (is_imei[rowindex]) {
      const imeiNumbers = $('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.imei-number').val();
      let html = '';
      if (!imeiNumbers) {
        html = `<div class="col-md-8 form-group imei-section">
        ...
        const arr = imeiNumbers.split(",");
        html = `<div class="col-md-8 form-group imei-section">
          <label>IMEI or Serial Numbers</label>
          <div class="table-responsive ml-2">
            <table id="imei-table" class="table table-hover"><tbody>`;
        for (let i=0;i<arr.length;i++){
          html += `<tr><td><input type="text" class="form-control imei-numbers" name="imei_numbers[]" value="${arr[i]}" /></td>
                   <td><button type="button" class="imei-del btn btn-sm btn-danger">X</button></td></tr>`;
        }
      ...
      $("#editModal .modal-element").append(html);
    }

    const row_product_name = $('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('td:nth-child(1)').text();
    const row_product_code = $('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('td:nth-child(2)').text();
    $('#modal_header').text(row_product_name + '(' + row_product_code + ')');

    $('input[name="edit_qty"]').val(num($(this).closest('tr').find('.qty').val()));
  ...
  $(document).on("click", "#imei-add-more", function() {
    var newRow = $("<tr>");
    newRow.append('<td><input type="text" class="form-control imei-numbers" name="imei_numbers[]" /></td>');
    newRow.append('<td><button type="button" class="imei-del btn btn-sm btn-danger">X</button></td>');
    $("table#imei-table tbody").append(newRow);
    var q = num($('input[name="edit_qty"]').val());
    $('input[name="edit_qty"]').val(q+1);
  });
  $(document).on("click", "table#imei-table tbody .imei-del", function() {
    $(this).closest("tr").remove();
    var q = num($('input[name="edit_qty"]').val());
    $('input[name="edit_qty"]').val(Math.max(0, q-1));
  });

  // ====== Edit modal (save) ======
  $('button[name="update_btn"]').on("click", function() {
    var id = $(this).attr('id');
    $('.ets-date'+id).val($('input[name="ets_date"]').val());
    $('.eta-date'+id).val($('input[name="eta_date"]').val());
    $('.etd-date'+id).val($('input[name="etd_date"]').val());
    $('.ship_term'+id).val($('input[name="ship_term"]').val());
    $('.ship_cost'+id).val($('input[name="ship_cost"]').val());
    $('.moq'+id).val($('input[name="moq"]').val());

    // reflect visible ship cost cell
    $('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.ship_costt').text(f(num($('input[name="ship_cost"]').val())));

    if (is_imei[rowindex]) {
      var imeiNumbers = '';
      $("#editModal .imei-numbers").each(function(i) {
        imeiNumbers += (i ? ',' : '') + $(this).val();
      });
      $('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.imei-number').val(imeiNumbers);
    }

    var edit_discount = num($('input[name="edit_discount"]').val());
    var edit_qty = num($('input[name="edit_qty"]').val());
    var edit_unit_cost = num($('input[name="edit_unit_cost"]').val());
  ...
  function checkQuantity(purchase_qty, flag) {
    ...
    var total_qty;
    if (operator[0] == '*') total_qty = purchase_qty * num(operation_value[0]);
    else if (operator[0] == '/') total_qty = purchase_qty / num(operation_value[0]);
  ...
  function calculateRowProductData(quantity) {
    ...
    var discPerUnit = num(product_discount[rowindex]);
    var discTotal   = discPerUnit * num(quantity);
    var tRate       = num(tax_rate[rowindex]);
    var $tr         = $('table.order-list tbody tr:nth-child('+(rowindex+1)+')');

    // tax_method: 1 = exclusive, else inclusive
    if (num(tax_method[rowindex]) == 1) {
      var net_unit_cost = row_product_cost - discPerUnit;
      var tax = net_unit_cost * num(quantity) * (tRate / 100);
      var sub_total = (net_unit_cost * num(quantity)) + tax;
      ...
      $tr.find('.subtotal-value').val(f(sub_total));
    } else {
      var sub_total_unit = row_product_cost - discPerUnit;
      var net_unit_cost = (100 / (100 + tRate)) * sub_total_unit;
      var tax = (sub_total_unit - net_unit_cost) * num(quantity);
      var sub_total = sub_total_unit * num(quantity);
  ...
  function calculateTotal() {
    // total qty
    var total_qty = 0;
    $(".qty").each(function(){ total_qty += num($(this).val()); });
    $("#total-qty").text(total_qty);
    $('input[name="total_qty"]').val(total_qty);

    // total discount
    var total_discount = 0;
    $(".discount").each(function(){ total_discount += num($(this).text()); });
    $("#total-discount").text(f(total_discount));
    $('input[name="total_discount"]').val(f(total_discount));

    // total tax
    var total_tax = 0;
    $(".tax").each(function(){ total_tax += num($(this).text()); });
    $("#total-tax").text(f(total_tax));
    $('input[name="total_tax"]').val(f(total_tax));

    // total subtotal (line totals)
    var total = 0;
    $(".sub-total").each(function(){ total += num($(this).text()); });
    $("#total").text(f(total));
    $('input[name="total_cost"]').val(f(total));
  ...
  function calculateGrandTotal() {
    ...
    var total_qty = num($('#total-qty').text());
    var subtotal = num($('#total').text());
    var order_tax_rate = num($('select[name="order_tax_rate"]').val());
    var order_discount = num($('input[name="order_discount"]').val());

    // sum per-line shipping
    let totalShippingCost = 0;
    $('input[name="ship_cost[]"]').each(function(){ totalShippingCost += num($(this).val()); });
    ...
    $('#subtotal').text(f(subtotal));
    $('#order_tax').text(f(order_tax));
    $('input[name="order_tax"]').val(f(order_tax));
    $('#order_discount').text(f(order_discount));
    $('#shipping_cost').text(f(totalShippingCost));
    $('#grand_total').text(f(grand_total));
    $('input[name="grand_total"]').val(f(grand_total));
  }

  // Order-level controls
  $('input[name="order_discount"]').on("input", calculateGrandTotal);
  $('input[name="shipping_cost"]').on("input", calculateGrandTotal); // (hidden in UI but kept)
  ...
  $('#purchase-form').on('submit',function(e){
    var rownumber = $('table.order-list tbody tr:last').index();
    if (rownumber < 0) { alert("Please insert product to order table!"); e.preventDefault(); return; }

    if($('select[name="status"]').val() == 2){
      let flag = 0;
      $(".qty").each(function() {
        rowindex = $(this).closest('tr').index();
        var quantity = num($(this).val());
        var recieved = num($('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.recieved').val());
        if (quantity != recieved){ flag = 1; return false; }
      });
      if(!flag){
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $(".product-code").each(function(i) {
          if ($(this).val() == data[1]) {
            rowindex = i;
            var qty = num($('table.order-list tbody tr:nth-child('+(rowindex+1)+') .qty').val()) + 1;
        ...
        cols += '<td class="net_unit_cost_cell"></td>';
        ...
        cols += '<input type="hidden" class="net_unit_cost_input" name="net_unit_cost[]" />';
        cols += '<input type="hidden" class="discount-value" name="discount[]" />';
        cols += '<input type="hidden" class="tax-rate" name="tax_rate[]" value="'+ data[3] +'"/>';
        cols += '<input type="hidden" class="tax-value" name="tax[]" />';
        cols += '<input type="hidden" class="subtotal-value" name="subtotal[]" />';
        cols += '<input type="hidden" class="imei-number" name="imei_number[]" />';
        cols += '<input type="hidden" class="original-cost product-cost" value="'+ data[2] +'" />';
        ...
        product_cost.splice(rowindex, 0, num(data[2]));
        product_discount.splice(rowindex, 0, f(0));
        tax_rate.splice(rowindex, 0, num(data[3]));
        tax_name.splice(rowindex, 0, data[4]);
        tax_method.splice(rowindex, 0, num(data[5]));
        ...
        if (data[11]) $('table.order-list tbody tr:nth-child('+(rowindex+1)+')').find('.edit-product').click();
