...
<section>
    <div class="container-fluid">
        <a href="#" data-toggle="modal" data-target="#createModal" class="btn btn-info add-warehouse-btn"><i class="dripicons-plus"></i> {{__('Add Production')}}</a>
        <a href="#" data-toggle="modal" data-target="#importWarehouse" class="btn btn-primary add-warehouse-btn"><i class="dripicons-copy"></i> {{__('Import Production')}}</a>
    </div>
        ...
        <table id="warehouse-table" class="table" style="width: 100%">
            <thead>
                <tr>
                    ...
                    <th>{{__('db.Email')}}</th>
                    <th>{{__('db.Address')}}</th>
                    <th style="display: none;">{{__('db.Number of Product')}}</th>
                    <th style="display: none;">{{__('db.Stock Quantity')}}</th>
            ...
            <tbody>
                ...
                <tr data-id="{{$warehouse->id}}">
                    ...
                    <td style="display: none;">{{$number_of_product}}</td>
                    <td style="display: none;">{{$stock_qty}}</td>
                    <td>
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                ...
                                <li class="divider"></li>
                                {{ Form::open(['route' => ['production.destroy', $warehouse->id], 'method' => 'DELETE'] ) }}
                                <li>
...
<div id="createModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
  <div role="document" class="modal-dialog">
    <div class="modal-content">
        {!! Form::open(['route' => 'production.store', 'method' => 'post']) !!}   
...
<div id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
  <div role="document" class="modal-dialog">
    <div class="modal-content">
        {!! Form::open(['route' => ['production.update',1], 'method' => 'put']) !!}
      <div class="modal-header">
        <h5 id="exampleModalLabel" class="modal-title"> {{__('Update Production')}}</h5>
        <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
      </div>
      <div class="modal-body">
        <p class="italic"><small>{{__('db.The field labels marked with * are required input fields')}}.</small></p>
...
<div id="importWarehouse" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
  <div role="document" class="modal-dialog">
    <div class="modal-content">
        {!! Form::open(['route' => 'production.import', 'method' => 'post', 'files' => true]) !!}
      <div class="modal-header">
                ...
                action: function ( e, dt, node, config ) {
                    if(user_verified == '1') {
                        ...
                        if(warehouse_id.length && confirm("Are you sure want to delete?")) {
                            $.ajax({
                                type:'POST',
                                url:'production/deletebyselection',
                                data:{
