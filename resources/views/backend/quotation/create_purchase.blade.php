    ...
    <label>{{__('PO#')}}</label>
    <input type="text" name="po_no" class="form-control po_no" value="" />
    </div>
    </div>
    ...
    <input type="hidden" name="reference_no" value="{{ $lims_purchase_data->reference_no }}" class="form-control" />
    </div>
...
$product_data = DB::table('products')->find($product_purchase->product_id);
    ...
    $product_cost = ($product_purchase->net_unit_price + ($product_purchase->discount / $product_purchase->qty)) / $unit_operation_value[0];
    }
    else{
    ...
    <td class="recieved-product-qty d-none"><input type="number" class="form-control recieved" name="recieved[]" value="{{$product_purchase->qty}}" step="any"/></td>
...
@endforeach
    ...
    <input type="hidden" class="net_unit_cost" name="net_unit_cost[]" value="{{$product_purchase->net_unit_price}}" />
   ...
   @endphp
    ...
    <div class="col-md-2">
    <div class="form-group">
    <input type="hidden" name="total_cost" value="{{$lims_purchase_data->grand_total}}" />
    </div>
    </div>
    <div class="col-md-2">
    <div class="form-group">
...
<div class="col-md-4 form-group">
<label>{{__('Ship Term')}}</label>
<input type="text" name="ship_term" class="form-control" placeholder="{{__(' ShipTerm')}}"/>
</div>
...
cols += '<input type="hidden" class="ship_cost'+flag2+'" name="ship_cost[]" />';
 ...
 $(document).ready(function () {
            ...
            $.ajax({
                url: "{{ route('generate.po_no') }}",
                type: "GET",
                success: function (res) {
                    
                    $('input[name="po_no"]').val(res.po_no)
                },
                error: function () {
                    alert('Could not generate PO number');
                }
            });
