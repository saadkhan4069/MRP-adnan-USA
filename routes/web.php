...
use App\Http\Controllers\wproductionController;
...
Route::group(['middleware' => ['common', 'auth', 'active']], function() {
    ...
    Route::controller(wproductionController::class)->group(function () {
        Route::post('importproduction', 'importproduction')->name('production.import');
        Route::post('production/deletebyselection', 'deleteBySelection');
        Route::get('production/lims_production_search', 'limsproductionSearch')->name('production.search');
        Route::get('production/all', 'productionAll')->name('production.all');
    });
    Route::resource('production', wproductionController::class);
    ...
    Route::controller(QuotationController::class)->group(function () {
        Route::prefix('quotations')->group(function () {
            ...
            Route::get('{id}/create_purchase_old', 'createPurchase_old')->name('quotation.create_purchase_old');
    ...
    Route::get('quotations/pdf/{id}/{id2}', [QuotationController::class, 'generatePdf']);
    Route::get('purchases/pdf/{id}/{id2}', [PurchaseController::class, 'generatePdf']);
    Route::get('sale/pdf/{id}/{id2}', [SaleController::class, 'generatePdf']);

 // Shipped list routes inside web.php
    Route::post('purchases/check-shipment-dates', [PurchaseController::class, 'checkShipmentDates']);
    Route::post('purchases/shipped-check', [PurchaseController::class, 'ShippedCheck']);
    Route::post('purchases/RawMterial', [PurchaseController::class, 'getRawMterial']);
    Route::get('purchases/getproduct/{id}',[PurchaseController::class, 'getProduct']);
Route::get('shipped/list', [PurchaseController::class, 'shippedIndex'])->name(
    'backend.purchase.shippedview');
Route::post('shipped/data', [PurchaseController::class, 'getShippedData'])->name('shipped.data');

Route::get('/shipped/child-details', [PurchaseController::class, 'childDetails']);

Route::post('/shipment/update', [PurchaseController::class, 'shippmnetUpdate'])->name('shipment.update');

Route::get('/shipment/modal/view', [PurchaseController::class, 'shipmentModal']);

Route::get('/generate-po-no', [PurchaseController::class, 'generatePoNoQT'])->name('generate.po_no');

Route::get('shipments/po-search', [ShipmentController::class, 'poSearch'])->name('shipments.po_search');
...
Route::get('shipment/{shipment}', [PurchaseController::class, 'shipmentShow'])
    ->name('shipment.show');
 // Shipped list routes inside web.php  END---------
Route::get('shipment/{shipment}/edit', [PurchaseController::class, 'shipmentedit'])->name('shipment.edit');
...
// Create label (modal submit)
Route::post('shipment/{shipment}/label', [PurchaseController::class, 'shipmentLabelCreate'])
    ->name('shipment.label.store');
...
Route::get('/products/materials/sample.csv', [CatalogController::class, 'sampleCsv'])
  ->name('materials.sampleCsv');

Route::post('/products/{product}/materials/import-csv', [CatalogController::class, 'importCsv'])
  ->name('materials.importCsv');

Route::prefix('products/{product}')->group(function () {
    Route::get('/catalog', [CatalogController::class, 'show'])->name('catalog.show');
 
    Route::post('/materials', [CatalogController::class, 'storeMaterial'])->name('materials.store');
    Route::put('/materials/{material}', [CatalogController::class, 'updateMaterial'])->name('materials.update');
    Route::delete('/materials/{material}', [CatalogController::class, 'destroyMaterial'])->name('materials.destroy');

    Route::post('/specs', [CatalogController::class, 'saveSpecs'])->name('catalog.specs.save');
    Route::post('/nutrition', [CatalogController::class, 'saveNutrition'])->name('catalog.nutrition.save');
});

Route::post('/raw-materials', [CatalogController::class, 'rawMaterialStore'])->name('raw-materials.store');

// optional: Vendors CRUD (simple)
Route::post('/vendors', [CatalogController::class, 'vendorStore'])->name('vendors.store');

//--------------------Catelog-----------------------------------

// Route::get('/products/{product}/catalog', [ProductController::class, 'productmaterialshow'])->name('products.catalog');
// Route::post('/products/{product}/materials', [ProductController::class, 'storeMaterial'])->name('materials.store');
// Route::put('/products/{product}/materials/{material}', [ProductController::class, 'updateMaterial'])->name('materials.update');
// Route::delete('/products/{product}/materials/{material}', [ProductController::class, 'destroyMaterial'])->name('materials.destroy');
// -------------------Product Catelog End----------------------------------------------
   
    Route::controller(PurchaseController::class)->group(function () {
        Route::prefix('purchases')->group(function () {
            Route::post('purchase-data', 'purchaseData')->name('purchases.data');
            Route::post('export-all', [PurchaseController::class, 'exportAll']);
    ...
    Route::controller(InventoryMovementController::class)->group(function () {
        Route::prefix('inventory-movement')->group(function () {
            Route::get('/', 'index')->name('inventory-movement.index');
            Route::post('movement-data', 'movementData')->name('inventory-movement.data');
            Route::post('get-product-stock', 'getProductStock')->name('inventory-movement.stock');
        });
    });
    ...
    Route::controller(ScheduleController::class)->group(function () {
        Route::prefix('schedule')->group(function () {
            // User Appointments
            Route::get('/my-appointments', 'userAppointments')->name('schedule.user-appointments');
            Route::post('/reschedule/{id}', 'reschedule')->name('schedule.reschedule');
            Route::post('/cancel/{id}', 'cancel')->name('schedule.cancel');
            
            // Admin Routes
            Route::get('/admin', 'adminAppointments')->name('schedule.admin');
            Route::post('/update-status/{id}', 'updateStatus')->name('schedule.update-status');
            Route::delete('/delete/{id}', 'destroy')->name('schedule.delete');
        });
    });
...
Route::controller(ScheduleController::class)->group(function () {
    Route::get('/schedule/booking', 'publicBooking')->name('schedule.public');
    Route::post('/schedule/available-slots', 'getAvailableSlots')->name('schedule.available-slots');
    Route::post('/schedule/book', 'storeAppointment')->name('schedule.book');
});

// Debug Route - Check Appointments (Remove in production)
Route::get('/schedule/debug/{date?}', function($date = null) {
    if (!$date) {
        $date = date('Y-m-d');
    }
    
    $appointments = \App\Models\Appointment::where('appointment_date', $date)
        ->whereIn('status', ['pending', 'confirmed'])
        ->get(['id', 'appointment_date', 'appointment_time', 'slot', 'status', 'first_name'])
        ->toArray();
    
    $bookedSlots = \App\Models\Appointment::getBookedSlots($date);
    $allSlots = \App\Models\Appointment::getTimeSlots();
    ...
    return response()->json([
        ...
        'all_available_slots' => $allSlots,
        'note' => 'Compare booked_times with all_available_slots keys to see if they match'
    ]);
})->name('schedule.debug');
